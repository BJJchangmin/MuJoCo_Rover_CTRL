...
void AlignAndScaleView(mj::Simulate* sim, const mjModel* m) {
  ...
  // otherwise use default free camera
    ...
    // Initial free-camera view for the rover.
    // This is applied only when the view is aligned/model is first loaded,
    // so the camera remains freely controllable afterward.
    sim->cam.azimuth = -114.5;
    sim->cam.distance = 2.644289;
    sim->cam.elevation = -19.75;
    sim->cam.lookat[0] = 0.0;
    sim->cam.lookat[1] = 0.0;
    sim->cam.lookat[2] = 0.1;
  }
}
  ...
  while (!this->platform_ui->ShouldCloseWindow() && !this->exitrequest.load()) {
    {
      ...
      //! Camera View Setting 뿐 아니라 simulation의 visual 및 기능들의 dafault 값들을 여기서 설정 할 수 있음

      //* 카메라를 원하는 각도에서 고정해서 차량을 따라 다니게 만들고 싶다면
      //* 1 : 아래 printf 주석해제하고 자유롭게 카메라 설정을 킨다음 원하는 각도를 찾습니다. cout에 해당하는 값들이 나올겁니다.
      // printf("Current Camera View: %f, %f, %f, %f, %f, %f \n", this->cam.azimuth,
      //        this->cam.distance, this->cam.elevation, this->cam.lookat[0], this->cam.lookat[1],
      //        this->cam.lookat[2]);

      //* 2 : 원하는 각도를 찾았다면 아래 주석을 풀고 해당 각도를 입력합니다. 하면 차량을 따라다니면서 원하는 각도로 고정됩니다.
      //* 단점은 고정하면 시뮬레이션 중에 회전이 되지 않습니다.
      // this->cam.type = mjCAMERA_TRACKING;
      // this->cam.fixedcamid = -1;
      ...
      //*자유롭게 카메라 설정을 원한다면 이것만
      this->cam.type = mjCAMERA_FREE;

      //* Option을 어떻게 사용하는지는 MuJoCo API를 참고하면 모든게 나옵니다.
      //* Visual Options *//
