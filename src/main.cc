...
HighLevelController<float> high_level_ctrl(robot);  // high level controller
...
// Create simulation data and start from the named home keyframe when available.
mjData* MakeDataWithHomeKeyframe(mjModel* model) {
  if (!model) {
    return nullptr;
  }
...
void Motor_ID_Setting(mjData * d)
{
   ...
   * * 4.
   */
  d->ctrl[0] = 0.001*traj_generator.system_ID(d->time);
  robot.joint_torque_des_[0][0] = d->ctrl[0];
...
void apply_joint_control(mjData * d)
{
  ...
  // Legacy v1 approximates the suspension linkage by overwriting joint angles.
  // rover_v2 closes the real four-bar loops with equality/connect constraints,
  // so applying this approximation there would fight the constraint solver.
  if (mj_name2id(m, mjOBJ_JOINT, "FL_sus_act") >= 0)
  {
    d->qpos[8] =  -d->qpos[7];
    d->qpos[12] = -d->qpos[11];
    d->qpos[16] = -d->qpos[15];
...
void Mu_Lambda_setting()
{
  ...
  if (d->time > 4)
  {
    for (size_t i = 0; i < 4; i++)
    {
      static const char* tire_geom_names[4] = {
        "tire_FL", "tire_FR", "tire_RL", "tire_RR"
      };
      const int tire_geom_id = mj_name2id(m, mjOBJ_GEOM, tire_geom_names[i]);
      if (tire_geom_id >= 0)
      {
        m->geom_friction[3*tire_geom_id] = estimate_param_ptr->mu_[i];
      }
      else
      {
        // Preserve the original v1 geom layout until its tire geoms are named.
        const int legacy_friction_index = 36 + 7*3*i;
        if (legacy_friction_index < 3*m->ngeom)
        {
          m->geom_friction[legacy_friction_index] = estimate_param_ptr->mu_[i];
        }
      }
...
void PhysicsLoop(mj::Simulate& sim) {
  ...
  {
    // Exit simulation if time exceeded the specified end time // => 이건 custom 아님. 사용 X
    ...
    if (sim.droploadrequest.load()) {
      ...
      mjData* dnew = nullptr;
      if (mnew) dnew = MakeDataWithHomeKeyframe(mnew);
      if (dnew) {
