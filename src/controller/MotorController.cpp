...
void MotorController<T>::drive_motor_control()
{
  ...
  for(size_t i = 0; i < 4; i++)
  {
    robot_.joint_torque_des_[i][2] = saturation_block(120.8, -120.8, pid_out_drive_[i][0]);
...
void MotorController<T>::steer_motor_control()
{
  ...
  for(size_t i = 0; i < 4; i++)
  {
    robot_.joint_torque_des_[i][1] = saturation_block(9.87,-9.87, pid_out_steer_[i][0]);
...
void MotorController<T>::suspension_motor_control()
{
  ...
  for(size_t i = 0; i < 4; i++)
  {
    robot_.joint_torque_des_[i][0] = saturation_block(44.82,-44.82, pid_out_sus_[i][0]);
...
void MotorController<T>::M_PID_drive()
{
  ...
  {
    // Gain Setting
    K[i] = PI_Gain_velctrl(J_drive_[i], B_drive_[i], 1, 3);
...
void MotorController<T>::M_PID_steer()
{
  ...
  {
    // Gain Setting
    K[i] = PD_Gain_posctrl(J_steer_[i], B_steer_[i], 1, 3);
...
void MotorController<T>::M_PID_sus()
{
  ...
  {
    // Gain Setting
    K[i] = PD_Gain_posctrl(J_sus_[i], B_sus_[i], 7, 10);
