...
Vector3d euler_angle_from_quaternion(double qw, double qx, double qy, double qz)
{
    /**
     * @brief Convert Quaternion to Euler Angle(ZYX)
     * @param qw, qx, qy, qz : Quaternion
     * @return euler_angle : Vector3d(roll, pitch, yaw)
     */
    ...
    euler_angle(0) = std::atan2(sinr_cosp, cosr_cosp);

    // pitch (y-axis rotation)
    double sinp = 2 * (qw * qy - qz * qx);
    if (std::abs(sinp) >= 1)
        euler_angle(1) = std::copysign(M_PI / 2, sinp); // use 90 degrees if out of range
    else
        euler_angle(1) = std::asin(sinp);

    // yaw (z-axis rotation)
    double siny_cosp = 2 * (qw * qz + qx * qy);
    double cosy_cosp = 1 - 2 * (qy * qy + qz * qz);
    euler_angle(2) = std::atan2(siny_cosp, cosy_cosp);
